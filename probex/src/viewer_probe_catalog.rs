...
fn detect_tracefs_events_root() -> ProbeCatalogResult<&'static Path> {
...
fn tracepoint_format_path(events_root: &Path, category: &str, name: &str) -> PathBuf {
...
fn discover_tracepoints(events_root: &Path) -> ProbeCatalogResult<Vec<(String, String)>> {
    ...
    for category_entry in fs::read_dir(events_root)? {
        ...
        for probe_entry in fs::read_dir(category_entry.path())? {
            ...
            let format_path = tracepoint_format_path(events_root, &category_name, &probe_name);
...
fn parse_field_declaration(declaration: &str) -> ProbeCatalogResult<(String, String)> {
...
fn parse_tracepoint_format_fields(
    format_contents: &str,
) -> ProbeCatalogResult<Vec<ProbeSchemaField>> {
...
fn load_tracepoint_fields(schema: &ProbeSchema) -> ProbeCatalogResult<Vec<ProbeSchemaField>> {
    ...
    let events_root = detect_tracefs_events_root()?;
    let format_path = tracepoint_format_path(events_root, &schema.target, &schema.probe);
    ...
    let fields = parse_tracepoint_format_fields(&format_contents)?;
    cache
        .lock()
        .map_err(|_| IoError::other("failed to lock tracepoint field cache"))?
